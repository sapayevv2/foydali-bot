...
add_acc_keyboard = ReplyKeyboardMarkup(
    keyboard=[[KeyboardButton(text="📞 Telefon orqali")], [KeyboardButton(text="⬅️ Bosh menyu")]],
    resize_keyboard=True
)

phone_keyboard = ReplyKeyboardMarkup(
    keyboard=[[KeyboardButton(text="📱 Telefon raqamni yuborish", request_contact=True)], [KeyboardButton(text="⬅️ Bosh menyu")]],
    resize_keyboard=True
)
...
def get_start_text(first_name: str) -> str:
    return f"🤖 **FOYDALI Bot ga xush kelibsiz, {first_name}!**\n\nKerakli bo'limni pastdagi tugmalardan tanlang 👇"
...
async def start_and_back_handler(message: types.Message):
    ...
    await message.answer(get_start_text(message.from_user.first_name), reply_markup=main_keyboard, parse_mode="Markdown")

@dp.message(F.text == "👨‍💻 Bot owner")
async def owner_handler(message: types.Message):
    await message.answer("👨‍💻 **Bot Dasturchisi va Egasiga Bog'lanish:**\n\nTelegram: @sapayevv2", reply_markup=main_keyboard, parse_mode="Markdown")
...
async def remove_account_handler(message: types.Message):
    ...
    await message.answer(text, reply_markup=no_acc_keyboard, parse_mode="Markdown")

@dp.message(F.text == "➕ Yangi akkaunt qo'shish")
async def add_account_handler(message: types.Message):
    user_id = message.from_user.id
    if user_id in connected_accounts and connected_accounts[user_id]:
        await message.answer("⚠️ **Sizda allaqachon akkaunt ulangan!**", reply_markup=has_acc_keyboard)
        return
    await message.answer("➕ **Yangi akkaunt ulash**\n\nAkkauntni ulash uchun quyidagi tugmani bosing:", reply_markup=add_acc_keyboard, parse_mode="Markdown")

@dp.message(F.text == "📞 Telefon orqali")
async def phone_option_handler(message: types.Message):
    await message.answer("📱 **Telefon orqali ulash**\n\nTelefon raqamingizni yuboring (masalan: `+998901234567`)\nYoki pastdagi tugma orqali yuboring:", reply_markup=phone_keyboard, parse_mode="Markdown")
...
async def process_phone_number(message: types.Message, phone: str):
    ...
    try:
        ...
        user_data[user_id] = {"client": client, "phone": phone, "phone_code_hash": sent_code.phone_code_hash, "step": "code"}
        await message.answer(f"✅ Raqam qabul qilindi: `{phone}`\n\n📩 Telegram'ingizga yuborilgan tasdiqlash kodini kiriting.\n*(Kodni 1-2-3-4-5 ko'rinishida yuboring)*", reply_markup=cancel_keyboard, parse_mode="Markdown")
...
async def finalize_login(message: types.Message, client: Client, user_id: int, phone: str):
    ...
    connected_accounts[user_id] = {"first_name": me.first_name, "phone": phone, "id": me.id, "username": f"@{me.username}" if me.username else "Mavjud emas"}
    del user_data[user_id]
    ...
    await message.answer("⚡ **.u funksiyasi faollashdi!**\n\nEndi istalgan guruhda `.u` yozsangiz bittalab mention qiladi. `.su` yozsangiz to'xtaydi.", reply_markup=main_keyboard)
